...
function mergeWithRequired(extracted) {
  ...
  // Append any extra parameters from the backend not in the required list
  const extraRows = (extracted || [])
    .filter(item => {
      const name = item.parameter_name || item.parameter || item.name || "";
      // Include if found/value exists and key is not already in required list
    ...
    .map(item => {
      ...
      return {
        ...
        available: true,
        extra: true,   // flag so UI can optionally style differently
      };
    });
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column", background: C.bg, borderLeft: isMobile ? "none" : `1px solid ${C.border}`, fontFamily: F.sans }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px 14px" }}>
        ...
        {!loading && !polling && params.map((r, i) => {
          ...
          // Show a divider label before the first extra (non-required) parameter
