    ...
    if (found) {
      ...
      const sources = found.sources?.length
        ...
        : (found.source?.document || found.source?.pages?.length)
          ...
          : [];
      // Treat 0% confidence with no value as "not found" — prevents misleading display
      const hasValue = !!(found.value ?? found.value_text);
      const isActuallyFound = hasValue && conf > 0;
      return {
        label: req.label,
        key: req.key,
        unit: req.unit,
        ...
        value: hasValue ? (found.value ?? found.value_text ?? "-") : null,
        confidence: isActuallyFound ? conf : null,
        notes: found.notes || null,
        sources: isActuallyFound ? sources : [],
        section: isActuallyFound ? (sources[0]?.section ?? found.source?.section ?? null) : null,
    ...
    const fetchParams = () => {
      api.getParameters(token, projectId)
        .then(data => {
          ...
          // Surface backend error messages immediately
          if (data.processing_status === "failed" && data.error_message) {
        ...
        .catch(() => {
          if (cancelled) return;
          consecutiveErrors++;
          // Exponential backoff on errors: 3s → 6s → 12s → 15s cap
        ...
        <div onClick={() => setPopup(null)}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div style={{ padding: "16px 18px 0" }}>
              ...
              {/* Sources — clickable: opens document at the referenced page */}
              {popup.sources?.length > 0 && (
                <div style={{ display: "flex", flexDirection: "column", gap: 5, marginBottom: 14 }}>
                  {popup.sources.map((src, idx) => {
                    ...
                    const openDoc = (page) => {
                      if (!docId) return;
                      const url = api.getDocumentFileUrl(projectId, docId);
                      window.open(page ? `${url}#page=${page}` : url, "_blank");
                    };
                    return (
                      <div key={idx}
                        onClick={() => canOpen && openDoc(firstPage)}
                        style={{ display: "flex", alignItems: "center", gap: 8, padding: "7px 10px", background: C.bg, borderRadius: 7, border: `1px solid ${C.border}`, cursor: canOpen ? "pointer" : "default", transition: "border-color 0.15s" }}
                        onMouseEnter={e => { if (canOpen) e.currentTarget.style.borderColor = C.green; }}
                        onMouseLeave={e => { e.currentTarget.style.borderColor = C.border; }}>
                        <span style={{ fontSize: 13 }}>{fileIcon(src.document?.split(".").pop())}</span>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 11, fontWeight: 600, color: canOpen ? C.green : C.text1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={canOpen ? `Click to open ${src.document}` : src.document}>
                        ...
                        <div style={{ display: "flex", gap: 3, flexShrink: 0, flexWrap: "wrap" }}>
                          {(src.pages || []).map(pg => (
                            <span key={pg}
                              onClick={(e) => { if (canOpen) { e.stopPropagation(); openDoc(pg); } }}
                              style={{ padding: "2px 6px", background: C.greenSubtle, border: `1px solid ${C.greenBorder}`, borderRadius: 4, fontSize: 10, fontWeight: 600, color: C.green, cursor: canOpen ? "pointer" : "default" }}
                              title={canOpen ? `Open page ${pg}` : `Page ${pg}`}>
                              Pg.{pg}
                            </span>
                          ))}
                        </div>
